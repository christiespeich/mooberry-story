 ...
 * @since 3.0
 */
abstract class Mooberry_Story_Settings {
	...
	protected function display_tabs() {
		do_action('mbds_settings_before_tabs', $this->page, $this->tab, $this->tabs );
		...
		echo '</h2>';
		do_action('mbds_settings_after_tabs', $this->page, $this->tab, $this->tabs );
	}
	...
	public function admin_page_display() {
		...
		// instructions
		do_action('mbds_settings_before_instructions', $this->page, $this->tab);

		do_action('mbds_settings_after_instructions');

		// tabs
		do_action('mbds_settings_pre_tab_display', $this->page, $this->tab);
		if ( array_key_exists( $this->page, $this->tabs ) && count($this->tabs[ $this->page ] ) > 0  ) {
			$this->display_tabs();
		}

		do_action('mbds_settings_post_tab_display', $this->page, $this->tab);


	/*	// About Mooberry boxes

		echo '<div id="mbds_about_mooberry"><div class="mbds_box">			<h3>' . __('Need help with Mooberry Story?', 'mooberry-story') . '</h3>';
...
*/
		...
		echo '<div class="wrap cmb2-options-page mbds_options ' . $this->key . '">';


		do_action('mbds_settings_before_metabox', $this->page, $this->tab, $this->metabox_id);
		...
		do_action('mbds_settings_after_metabox', $this->page, $this->tab, $this->metabox_id);
	...
	function add_options_page_metabox() {
		...
		$mbds_settings_metabox = new_cmb2_box( array(
			'id'         => $this->metabox_id,
			'hookup'     => false,
			'cmb_styles' => false,
			'show_on'    => array(
		...
		$this->metabox = $mbds_settings_metabox;

		$mbds_settings_metabox = apply_filters('mbds_settings_metabox', $mbds_settings_metabox, $this->page, $this->tab);
	}


	public final function uniqueID_generator( $value = '' ) {
		//return apply_filters('mbds_settings_uniqid', Mooberry_Story()->helper_functions->uniqueID_generator( $value ) );
